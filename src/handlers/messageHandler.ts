...
let commandMap = new Map<string, (args: string, voiceManager:VoiceManager, client:Client) => string>([
	["clear", clearParty],
	["kick", kick],
	["clearall", clearAll],
]);
...
let MessageHandler =
	(client: Client, voiceManager: VoiceManager) => (message: Message) => {
		...
		if (
			modRoleIds.filter((id) => message.member.roles.cache.has(id)).length > 0 &&
			...
			message.reply(
				"you are not a mod or are using this command in the wrong channel"
			);
		}
	};
